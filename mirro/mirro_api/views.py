...
def users(request):
    if request.method == 'POST':
        ...
        if not username or username == ' ':
            errors.append({'title':'username','message':'Поле не должно быть пустым'})
        elif not username.isalpha() or not username.isascii():
            errors.append({'title':'username','message':'Поле должно содержать только латиницу'})

        if not password or password == ' ':
            errors.append({'title':'password','message':'Поле не должно быть пустым'})
        elif len(password) < 8 or not any(not char.isalnum() for char in password) or not any(char.isdigit() for char in password):
            errors.append({'title':'password','message':'Пароль должен быть больше 8 символов, должен содержать спецсимволы и цифры'})

        if not email or email == ' ':
            errors.append({'title':'email','message':'Поле не должно быть пустым'})
        try:
            user = User.objects.get(email=email)
        except User.DoesNotExist:
            user = None
        if user:
            errors.append({'title':'email','message':'Пользователь с таким email уже существует'})
        ...
        if errors:
            ...
            print(error422)
            return JsonResponse(error422, safe=False, status=401)
...
def auth(request):
    if request.method == 'POST':
        ...
        if not user:
            errors.append({'title':'email','message':'Пользователь с таким email не существует'})

        hasher = PBKDF2PasswordHasher()
        if user and not hasher.verify(password, user.password):
            errors.append({'title':'password','message':'неверный пароль'})
...
def boards(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse({'code':'200','data':boards_list}, safe=False, status=200)
...
def board_id(request,id_board):
    ...
    user=is_auth(request)
    if not user: return JsonResponse({'code': 401,'message': 'Пользователь не авторизирован'}, safe=False, status=401)
    ...
    if request.method in ['PUT', 'PATCH']:
        ...
        if title:
            ...
            mass['title']=title
        if is_published:
            if is_published not in [1,0]: return JsonResponse({'code':'422','message':'проваленнв валидация данных','data':is_published},safe=False,status=422)
            mass['is_published']=is_published
...
def boards_id_accesses(request, id_board):
    ...
    if request.method == 'GET':
        acces = AccessToEdit.objects.filter(author=0,fk_board=board).values()
        return JsonResponse({'code':200,'message':'список соавторов','data':list(acces)},safe=False,status=200)
    ...
    if request.method == 'POST':
        ...
        if created:return JsonResponse({'code':'201','message':'пользователь создан','data':{'pk_access_to_edit':access.pk_access_to_edit}},safe=False,status=201)
        return JsonResponse({'code':'422','message':'проваленна валидация данных'},safe=False,status=422)
    ...
    if request.method == 'DELETE':
        ...
        if user == email_user: return JsonResponse({'code':'403','message':'недостаточно прав'},safe=False,status=403)
...
def boards_id_likes(request, id_board):
    ...
    if request.method == 'POST':
        _,access=Like.objects.get_or_create(fk_user=user,fk_board=board)
...
def logout(request):
    logout(request)
    return JsonResponse({'code': 200, 'message': 'logout ok'},status=200)